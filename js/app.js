...
(function () {
  ...
  async function renderHome(view) {
    ...
    const phone = el('<div class="card"></div>');
    phone.innerHTML =
      '<div class="card-title">' + vnFlag('vn-flag sm') + ' 越南语学习 · 今日速览</div>' +
      '<div class="phone-mock"><div class="phone-notch"></div>' +
      '<div class="phone-screen">' +
      '<div class="pm-head">今日必背词</div>' +
      '<div class="pm-words">' + VN_WORDS.slice(0, 6).map(w => '<div class="pm-word"><b>' + esc(w.vn) + '</b><span>' + esc(w.zh) + '</span></div>').join('') + '</div>' +
      '<div class="pm-foot">点开「越南语学习」开始跟读 🔊</div>' +
      '</div></div>';
    view.append(phone);
  ...
  async function renderDaily(view) {
    ...
    // 每日默认日课：今天日常为空时铺一套；并确保「越南语学习 30 分钟」始终在（用户要求）
    async function ensureRoutine() {
      ...
      if (!tasks.some(t => t.category === 'routine' && /越南语学习/.test(t.text || ''))) {
        await DB.put('tasks', { date: todayStr(), text: '越南语学习 30 分钟', category: 'routine', tag: '每日必做', done: false, createdAt: Date.now() });
      }
    }
    await ensureRoutine();
    refresh();
  ...
  async function renderViet(view) {
    ...
    // 阶段路线
    const sbox = $('#vnStages');
    VN_STAGES.forEach((s, i) => {
      const done = doneStages.includes(i);
      const curStage = i === cur;
      const locked = i > cur;
      const row = el('<div class="stage-row' + (done ? ' done' : '') + (locked ? ' locked' : '') + '"></div>');
      row.innerHTML = '<div class="stage-head"><div class="stage-dot">' + (done ? '✓' : (i + 1)) + '</div>' +
        '<div style="flex:1"><b>' + esc(s.t) + '</b>' + (curStage ? '<span class="stage-cur">学习中</span>' : '') +
        '<div class="muted" style="font-size:12.5px;margin-top:2px">' + esc(s.d) + '</div></div>' +
        (done ? '' : '<button class="btn sm ghost vn-master" data-i="' + i + '">✓ 标记掌握</button>') + '</div>';
      if (!done && !locked) row.querySelector('.vn-master').onclick = async () => {
        const arr = doneStages.slice(); arr.push(i);
        await DB.put('meta', { id: 'vnStages', value: arr }); renderViet(view);
      };
      sbox.append(row);
    });
    ...
    VN_START.steps.forEach(s => {
      const c = el('<div class="vn-step"></div>');
      c.innerHTML = '<div class="vn-step-num">' + esc(s.icon) + '</div>' +
        '<div><b>' + esc(s.t) + '</b><div class="muted" style="font-size:13px;margin-top:2px">' + esc(s.d) + '</div></div>';
      $('#vnStart').append(c);
    });
    // 上路引导：第一周计划
    VN_START.week.forEach(w => {
      const c = el('<div class="vn-week-row"></div>');
      c.innerHTML = '<span class="vn-week-d">' + esc(w.d) + '</span><span class="vn-week-t">' + esc(w.t) + '</span>';
      $('#vnWeek').append(c);
    });

    // 完整字母表（点击朗读）
    function alphaCell(x, box) {
      const c = el('<button class="alpha-cell"></button>');
      c.innerHTML = '<span class="alpha-l">' + esc(x.l) + '</span><span class="alpha-c">' + esc(x.c) + '</span>';
      c.onclick = () => speakVi(x.l);
      box.append(c);
    }
    VN_VOWELS.forEach(x => alphaCell(x, $('#vowels')));
    VN_CONSONANTS.forEach(x => alphaCell(x, $('#consonants')));

    // 拼读小课堂
    const spellBox = $('#spell');
    VN_SPELL.forEach(s => {
      const c = el('<div class="spell-card"></div>');
    ...
    function paintFlash() {
      ...
      const box = $('#flash');
      box.innerHTML =
        '<div class="flash-card' + (flipped ? ' flipped' : '') + '" id="fcard"><div class="flash-inner">' +
          '<div class="flash-face flash-front"><span class="flash-tag">中文 · 想想越南语怎么说</span>' +
          '<div class="flash-zh">' + esc(w.zh) + '</div><div class="flash-tip">👆 点一下揭晓</div></div>' +
          '<div class="flash-face flash-back"><span class="flash-tag">越南语</span>' +
          '<div class="flash-vn">' + esc(w.vn) + '</div><button class="btn sm ghost fc-play">🔊 朗读</button></div>' +
        '</div></div>' +
        '<div class="row spread" style="margin-top:12px"><button class="btn sm ghost" id="fcPrev">← 上一张</button>' +
        '<span class="muted">' + (fi + 1) + ' / ' + VN_WORDS.length + '</span>' +
        '<button class="btn sm ghost" id="fcNext">下一张 →</button></div>';
      $('#fcard').onclick = (e) => { if (e.target.closest('.fc-play')) return; flipped = !flipped; paintFlash(); if (flipped) speakVi(w.vn); };
      const play = box.querySelector('.fc-play'); if (play) play.onclick = (e) => { e.stopPropagation(); speakVi(w.vn); };
      $('#fcPrev').onclick = () => { fi = (fi - 1 + VN_WORDS.length) % VN_WORDS.length; flipped = false; paintFlash(); };
      $('#fcNext').onclick = () => { fi = (fi + 1) % VN_WORDS.length; flipped = false; paintFlash(); };
    }
    paintFlash();

    // 实用场景短句
    const scenesBox = $('#scenes');
    VN_SCENES.forEach(sc => {
      const c = el('<div class="scene-block"></div>');
      c.innerHTML = '<div class="scene-name">' + esc(sc.icon) + ' ' + esc(sc.name) + '</div>';
      sc.items.forEach(it => {
    ...
    // 特殊字母卡
    const spBox = $('#special');
    VN_SPECIAL.forEach(x => {
      const c = el('<div class="vn-special"></div>');
      c.innerHTML = '<div class="vn-sp-letter">' + esc(x.l) + '</div><div><b>' + esc(x.c) + '</b><div class="muted" style="font-size:12.5px">' + esc(x.note) + '</div></div>';
      spBox.append(c);
    });

    // 声调卡
    const tonesBox = $('#tones');
    VN_TONES.forEach(t => {
    ...
    VN_UNITS.forEach((u, i) => {
      const c = el('<div class="unit-card"></div>');
      c.innerHTML = '<div class="row spread"><b>' + esc(u.t) + ' <span class="muted">' + esc(u.zh) + '</span></b><span class="chip">第' + (i + 1) + '课</span></div>' +
    ...
    // 跟练入口统一绑定
    view.querySelectorAll('.vn-go').forEach(b => b.onclick = () => openLink(b.dataset.url));

    // 推荐资源
    const resBox = $('#res');
    VN_RES.forEach(r => {
      const c = el('<div class="vn-res"></div>');
      c.innerHTML = '<span>' + esc(r.t) + '</span><button class="btn sm ghost vn-go" data-url="' + r.url + '">打开 ↗</button>';
      resBox.append(c);
    });
    resBox.querySelectorAll('.vn-go').forEach(b => b.onclick = () => openLink(b.dataset.url));

    function renderUnit(u) {
      if (window.speechSynthesis) window.speechSynthesis.cancel();
      let showZh = false;
      view.innerHTML =
        '<button id="back" class="btn ghost sm" style="margin-bottom:10px">← 返回课程</button>' +
        '<div class="art-detail-head"><b class="art-title">' + esc(u.t) + '</b><div class="muted">' + esc(u.zh) + '</div></div>' +
        '<div class="row" style="gap:6px;margin:10px 0;flex-wrap:wrap">' +
          '<button id="showZh" class="btn sm ghost">📖 显示译文</button>' +
          '<button id="all" class="btn sm ghost">🔊 朗读全部</button></div>' +
        '<div id="body" class="art-body"></div>' +
        (u.vocab && u.vocab.length ? '<div class="card" style="margin-top:12px"><div class="card-title">📝 本课词汇</div><div id="vocab"></div></div>' : '');
      const bodyEl = $('#body');
      function paint() {
        bodyEl.innerHTML = '';
      ...
      if (u.vocab && u.vocab.length) {
        const vb = $('#vocab');
        u.vocab.forEach(v => { const d = el('<div class="bil item"></div>'); d.innerHTML = '<div class="en">' + esc(v[0]) + '</div><div class="zh">' + esc(v[1]) + '</div>'; vb.append(d); });
      }
      paint();
    ...
    async function refresh() {
      ...
      logs.forEach(l => {
        ...
        card.querySelector('.del').onclick = async () => { if (await confirmDel('删除这条？')) { await DB.del('viet', l.id); refresh(); } };
        box.append(card);
      });
    }
    $('#add').onclick = async () => {
      const f = await promptForm('记录越南语学习', [
        { name: 'minutes', label: '学习分钟', type: 'number', placeholder: '如 15' },
        { name: 'note', label: '学了什么（选填）', type: 'textarea', placeholder: '单词 / 发音 / 课文' },
      ]);
      if (f) { await DB.put('viet', { date: todayStr(), minutes: f.minutes, note: f.note, createdAt: Date.now() }); refresh(); }
    };
    refresh();
  }
